 ...
 */
module.exports.copyTemplateDirectory = function copyTemplateDirectory(srcpath, destpath, context, ignore, keep) {
    var self = this;
    ...
    _.each(fs.readdirSync(fullSourcePath), function(srcEntryName) {
        ...
        if (self.isDirectory(fullEntryPath)) {
            ...
            self.copyTemplateDirectory(relSrcPath, destPath, context, ignore, keep);
...
module.exports.checkConfigExists = function checkConfigExists(env) {
  var destinationKey = env;
  if(destinationKey.includes('dev')) {
  ...
  var configEnv = fs.existsSync(configPath);

  if (!configEnv) throw '[-] No such environment has been configured: ' + configPath;
