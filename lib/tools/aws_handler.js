...
module.exports.ec2createSG = function ec2createSG(opts) {
  try {
    ...
    var sgAuthCmd = [];
    if (opts.ssl) {
      sgAuthCmd = 'aws ' + ingressRule(443, sgJson.GroupId).join(' ');
      this.execSync(sgAuthCmd, { cwd: cwd });
    } else {
      sgAuthCmd = 'aws ' + ingressRule(80, sgJson.GroupId).join(' ');
      this.execSync(sgAuthCmd, { cwd: cwd });
    }

    sgAuthCmd = 'aws ' + ingressRule(22, sgJson.GroupId).join(' ');
    this.execSync(sgAuthCmd, { cwd: cwd });
