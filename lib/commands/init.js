...
Command.create({
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    if (config.engines.graphql === 'apollo') {
      ...
      self.installNpmPackage('apollo-cache-inmemory', {cwd: appDirectory});
      self.installNpmPackage('apollo-client@1.0.0', {cwd: appDirectory});
      self.installNpmPackage('apollo-link', {cwd: appDirectory});
      self.installNpmPackage('apollo-link-http', {cwd: appDirectory});
      ...
      self.installNpmPackage('graphql-server-express', {cwd: appDirectory});
