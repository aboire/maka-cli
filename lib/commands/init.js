...
Command.create({
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    meteorInstallList.push('check ddp-rate-limiter accounts-password alanning:roles');

    if (config.engines.test === 'jasmine') {
      // install the jasmine driver package and html/console reporter and coverage
      meteorInstallList.push('sanjo:jasmine maka:html-reporter velocity:console-reporter dburles:factory lmieulet:meteor-coverage');
      npmDevInstallList.push('enzyme sinon enzyme-adapter-react-16 chai jquery');
      ...
      npmDevInstallList.push(' nightmare phantomjs-prebuilt enzyme sinon enzyme-adapter-react-16 chai jquery');
    }
    ...
    if (config.template.css) {
      if (config.engines.css == 'scss')
        meteorInstallList.push('fourseven:scss');
    ...
    npmInstallList.push('@babel/runtime@latest bcrypt');
    ...
    if (config.engines.client === 'react' || config.engines.client === 'reflux') {
      ...
      if (config.engines.test !== 'none') {
        npmInstallList.push('react@16.4.0 react-dom@16.4.0 react-test-renderer@16.4.0');
      } else {
        npmInstallList.push('react react-dom');
      }
      npmInstallList.push('react-router@3.2.1 react-addons-pure-render-mixin prop-types babel-plugin-transform-decorators-legacy');
    }
    ...
    if (config.engines.graphql === 'apollo') {
      npmInstallList.push('apollo-boost apollo-cache-inmemory apollo-client apollo-link apollo-link-http apollo-server-express express body-parser graphql merge-graphql-schemas react-apollo');
    ...
    if (config.engines.ssr === 'true' && config.engines.theme === 'material') {
      npmInstallList.push('jss react-jss jss-preset-default');
    }
