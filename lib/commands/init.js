...
Command.create({
  ...
  return CurrentConfig.withValue(config, function () {
    ...
    if (config.engines.graphql === 'apollo') {
      npmInstallList.push('apollo-cache-inmemory apollo-client apollo-link apollo-link-http apollo-link-schema apollo-link-ws apollo-live-client apollo-live-server apollo-morpher apollo-server-express apollo-upload-client graphql graphql-load graphql-tools graphql-type-json merge-graphql-schemas meteor-apollo-accounts react-apollo subscriptions-transport-ws uuid');
      meteorInstallList.push('accounts-password cultofcoders:apollo cultofcoders:apollo-accounts swydo:graphql');
    }
    ...
    if (config.engines.client === 'vanilla') {
      meteorRemoveList.push('blaze-html-templates reactive-var tracker');
