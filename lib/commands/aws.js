...
var AWS_COMMAND_DESCRIPTION = 'Create, view, and terminate EC2 instances.\n' +
  'Assign key-pairs and host IPs,\n' +
  'Describe, terminate, and reboot commands do not need an environment (env) option.\n\n' +
  'Example Workflow: \n' +
  ...
  'In order to deploy to an instance with NGINX using SSL, you must\n' +
  'create a new configuration that has the NGINX SSL config:\n' +
  ' Example Workflow for SSL: \n' +
  ' $ maka g:config staging --ssl\n' +
  ' $ maka ssl generate-csr --env staging # cert signing request\n' +
  ' $ maka ssl generate-ssc --env staging # self-signed certificate.\n' +
...
var createKey = function(config, cwd, destinationKey, context) {
  ...
  var fileNameFull = cwd + '/_keys/' + keyName + '-' + destinationKey + '.pem.txt';
  ...
  if (!context.confirm('[?] Would you like to update the ssh.json config file?')) {
    ...
    try {
      ...
      sshConfigJson.pem = '_keys/' + keyName + '-' + destinationKey + '.pem.txt';
...
Command.create({
   ...
   * Terminate Instance
   */
  if (args[0].includes('reboot-instance') || args[0].includes('ri')) {
    var instances = this.ec2describeInstances({cwd});
    var choice = this.ask('[?] Please choose an instance ID: ') || null;
    choice = (choice !== null) ? parseInt(choice) : choice;
    if (choice !== null && typeof choice === 'number') {
      ...
      if (instance.status === 'running') {
        var confirm = this.confirm('[!!] You are about to reboot instance: ' + instance.name + ' Ready?');
        if (confirm) {
          this.ec2rebootInstance({ cwd, instanceId: instance.instanceId });
        } else {
          this.logError('[-] You have aborted the reboot.  The instance: ' + instance.name + ' is still running.');
        }
      } else {
        this.logError('[-] There are no instances in a state that can be rebooted; either they are spinning up, or are already terminated.');
      }
    }
    return;
  }
  ...
  try {
    ...
    if (args[0].includes('create-instance') || args[0].includes('ci')) {
      ...
      try {
        ...
        var imgDef = this.execSync('aws ec2 describe-images --filters "Name=description,Values=*LTS*" "Name=name,Values=*server*" "Name=name,Values=*ubuntu*" "Name=creation-date,Values=*' + date + '*" "Name=root-device-type,Values=ebs" "Name=description,Values=*Ubuntu? ??\.?? LTS? amd64 bionic image*" --query "Images[*].{Desc:Description,ID:ImageId,alias:ImageOwnerAlias}"', {}, false);
        var imgDefJson = JSON.parse(imgDef);
        var imageList = [];
        var indexdList = [];
        imgDefJson.forEach(function(item, index) {
          if (item.alias === null) {
            var date = item.Desc.split('build on ')[1];
            imageList.push({ amiId: item.ID, desc: item.Desc, date});
          }
        });
        ...
        imageList.forEach(function(item, index) {
            indexdList.push({ id: index, amiId: item.amiId, desc: item.desc });
        });

        indexdList.forEach(function(item) {
      ...
      try {
        var subnet = JSON.parse(this.execSync('aws ec2 describe-subnets --filter "Name=subnet-id,Values=' + SUBNET_ID + '" --query "Subnets[*].{subnet:CidrBlock}"', {}, false))[0].subnet;
