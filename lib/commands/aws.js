...
Command.create({
  ...
  try {
    ...
    if (createKey) {
      ...
      var fileNameFull = cwd + '/_keys/' + name + '.pem.txt';
      var success = this.ec2key({ cwd: cwd, name, toFile: fileNameFull });
    ...
    if (createInstance) {
      // Get default VPC
      var vpcDef = this.execSync('aws ec2 describe-vpcs');
      ...
      try {
        var sgDef = this.execSync('aws ec2 describe-security-groups --query "SecurityGroups[*].{Name:GroupName,ID:GroupId}"');
